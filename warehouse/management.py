...
def decrease_stock(order_line: "OrderLine", quantity: int, warehouse_pk: str):
    ...
    logger.info(f'order line id :: {order_line.id}, allocated quantity :: {order_line.allocations}')
    try:
        stock = (
        ...
        error_context = {"order_line": order_line.id, "warehouse_pk": warehouse_pk}
        logger.info(f'Stock does not exist :: {order_line.variant}, error :: {error_context}')
    ...
    if stock.quantity - quantity_allocated < quantity:
        error_context = {"order_line": order_line.id, "warehouse_pk": warehouse_pk, "variant_id": order_line.variant_id}
        logger.info(f'Insufficient stock :: {order_line.variant}, error :: {error_context}')
        raise InsufficientStock(order_line.variant, error_context)

    stock.quantity = F("quantity") - quantity
    stock.save()
    logger.info(f'Stock details for order line after fulfillment :: {order_line.id}, stock quantity :: {stock.quantity}')
