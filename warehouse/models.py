...
from ..core.models import ModelWithCreateUpdateTimestamp
...
class Stock(ModelWithCreateUpdateTimestamp):
    ...
    def set_product_metadata_instock(self):
        stocks = (Stock.objects.select_related("product_variant")
                ...
                .filter(product_variant__product_id=self.product_variant.product_id, total_available__gt=0))
        
        if stocks:
            product = self.product_variant.product
            product.metadata.update({"instock":True})
            product.save()
        else:
            product = self.product_variant.product
            product.metadata.update({"instock":False})
            product.save()

    def save(self, *args, **kwargs):
        
        """Overwrite save to update product metadata instock key"""
        super().save(*args, **kwargs)
        self.set_product_metadata_instock()
