...
def get_thumbnail(image_file, size, method, rendition_key_set="products"):
    if image_file:
        ...
        if settings.FILE_STORAGE == 'AWS':
            filename, file_extension = os.path.splitext(image_file.name)
            url = 'https://{}.s3.amazonaws.com/{}/{}/{}-{}-{}{}'.format(
                settings.AWS_MEDIA_BUCKET_NAME,
                settings.AWS_LOCATION, settings.VERSATILEIMAGEFIELD_SETTINGS.get("sized_directory_name"),
                filename, method, used_size, file_extension)

        elif settings.FILE_STORAGE == 'GCP':
            filename, file_extension = os.path.splitext(image_file.name)

            url = 'https://storage.googleapis.com/{}/{}/{}/{}-{}-{}{}'.format(
                settings.GS_MEDIA_BUCKET_NAME,
                settings.GS_LOCATION,
                settings.VERSATILEIMAGEFIELD_SETTINGS.get("sized_directory_name"),
                filename, method, used_size, file_extension)
        
        elif settings.FILE_STORAGE == 'AZURE':
            filename, file_extension = os.path.splitext(image_file.name)
