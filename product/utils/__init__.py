...
def sourcing_request_status_update_notification(sourcing_request):

    store = sourcing_request.store
    product_name = sourcing_request.product.name

    brand_name = sourcing_request.brand.brand_name
    brand_slug = sourcing_request.brand.slug

    all_users = store.get_store_authorized_users()

    if len(all_users)>0:
        user = all_users[0]
        user_id = user.id
        context_variables = dict()
        context_variables[user_id] = {'brand_name':brand_name,'product_name':product_name,'brand_slug':brand_slug}
        send_notifications([user],context_variables,event_code="EV_sourcing_request_update")

def sourcing_request_coupon_created_notification(sourcing_request):
    ...
    if len(all_users)>0:
        ...
        user_id = user.id
        context_variables = dict()
        context_variables[user_id] = {'brand_name':brand_name,'product_name':product_name}
        send_notifications([user],context_variables,event_code="EV_sourcing_request_coupon_created")
