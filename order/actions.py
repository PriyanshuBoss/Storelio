...
def order_fulfilled(
    ...
    logger.info(f'Order Status after fulfillment :: {order.id}, allocated quantity :: {order.status}')
...
def fulfill_order_line(order_line, quantity, warehouse_pk):
    ...
    logger.info(f'Fulfulling order line with order_line_id :: {order_line.id}, quantity :: {quantity}')
    ...
    logger.info(f'order line fulfilled with order_line_id :: {order_line.id}, quantity_fulfilled :: {order_line.quantity_fulfilled}')
    ...
    for line in lines:
        ...
        if quantity > 0:
            try:
                ...
                logger.exception(f"Error while fulfilling order line :: {order_line.id}, error :: {e}")
...
def create_fulfillments(
    ...
    for warehouse_pk in fulfillment_lines_for_warehouses:
        fulfillment = Fulfillment.objects.create(order=order, status=fulfillment_status)
        fulfillments.append(fulfillment)
        logger.info(f'Fulfillment created with id :: {fulfillment.id}')
    ...
    logger.info(f'Fulfillment lines created with id :: {[f_line.id for f_line in fulfillment_lines]}')
