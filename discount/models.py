...
from saleor.core.models import ModelWithCreateTimestamp, ModelWithCreateUpdateTimestamp, ModelWithMetadata

from ..core.permissions import DiscountPermissions
from ..core.utils.translations import TranslationProxy
from . import DiscountValueType, VoucherOwner, VoucherType
...
class VoucherQueryset(models.QuerySet):
    ...
    def is_applicable_on_store(self, store_id):
        
        return self.filter(store=store_id).exists()


class Voucher(ModelWithMetadata, ModelWithCreateTimestamp):
    type = models.CharField(
        max_length=25, choices=VoucherType.CHOICES, default=VoucherType.ENTIRE_ORDER
    )
    ...
    brands = models.ManyToManyField("brand.brand", blank=True)
    is_shipping = models.BooleanField(default=False)
    user = models.ForeignKey("account.User", on_delete=models.CASCADE, null=True)
    store = models.ForeignKey("store.StoreInfo", on_delete=models.CASCADE, null=True)
    owner = models.CharField(max_length=15,
                            choices=VoucherOwner.CHOICES,
                            default=VoucherOwner.ZAAMO)
    ...
    max_discount =  MoneyField(amount_field="max_discount_value", currency_field="currency")
    discount_rule = models.JSONField(default=dict, blank=True)
    ...
    def get_discount_amount_for(self, price: Money):
        ...
        discount_value = price - after_discount

        if self.max_discount and discount_value > self.max_discount:
            discount_value = self.max_discount

        return discount_value
    
    def voucher_error_msg(self):
        msg = f"This offer is only valid for orders over {amount(self.min_spent)}."
        if self.name=="COTD":
            msg = f"You need to add products worth Rs {amount(self.min_spent)} from Collection of the Day to apply this coupon"

        return msg

    def validate_min_spent(self, value: Money):
        if self.min_spent and value < self.min_spent:
            msg = self.voucher_error_msg()
    ...
    def check_voucher_app_code_compatibility(self,checkout_app_code):

        if self.metadata.get('app_code')=='ZS' and not checkout_app_code=='ZS':
            return False
...
class VoucherLogs(ModelWithCreateUpdateTimestamp):

    user = models.ForeignKey("account.User", on_delete=models.CASCADE, related_name="voucher_logs")
    store = models.ForeignKey("store.StoreInfo", on_delete=models.CASCADE, related_name="voucher_logs")
    voucher_code = models.CharField(default="", max_length=80)

    log_msg = models.TextField(default="", max_length=400)


class VoucherStoreDealMapping(ModelWithMetadata,ModelWithCreateUpdateTimestamp):
    
    voucher = models.ForeignKey(Voucher, related_name="voucher_store_deal", on_delete=models.CASCADE)
    store = models.ForeignKey("store.StoreInfo", related_name="voucher_store_deal", on_delete=models.CASCADE)    
