...
from .style_stree_helper import StyleStreeHelper
...
class StyleStreeImpl():
    ...
    def _fetch_product_list_from_style_stree_brandAPI(self,shop):

        try:
            product_list = []
            api = ApiClient(host=shop.get('store_url'), path=shop.get('products_api_endpoint'))
            api.add_url_param('page_size',100)
            page = 1
            count = 0
            ...
            while True:
                ...
                api.update_url_params({'page_number':page})
                
                api.get()
                resp = api.fetch_response()
                
                current_page_product_list = resp.get('items')
    ...
    def insert_mapped_product_data_to_mongo(self, product_data, store_name, store_url):
        mapped_product_list = []
        instance = StyleStreeBrandIntegeration()
    ...
    def update_brand_record_for_resync(self,product_list,brand_name,product_data_id_existing):
        ...
        for product in product_list:
            existing_product = product_data_id_existing.get(product['product_id'])
    ...
    def insert_product_data_from_style_stree_brand_store(self, shop,resync=False):
        
        response_context = dict()
        
        store_name = shop.get('name')
        store_url = shop.get('store_url')
        product_list = self._fetch_product_list_from_style_stree_brandAPI(shop)
        ...
        existing_product_data = self.fetch_product_data_from_style_stree_brand_store_by_name(store_name)
        ...
        if data_exist:
            ...
            for product in product_data_existing:
                product_data_id_existing[product['product_id']] = product
        ...
        if s_name:
            try:
                ...
                self._get_instance_of_mongo_connection().update_data({'store_url': store_url}, {'$set': post_data}, 'style_stree_products')
                self._get_instance_of_mongo_connection().delete_gridfs(store_name)
            
            except Exception as e:
                logger.exception(e)
                self._get_instance_of_mongo_connection().delete_data({'store_name': store_name}, 'style_stree_products')

        else:
            self._get_instance_of_mongo_connection().insert_data(post_data, 'style_stree_products')
    ...
    def fetch_product_data_from_style_stree_brand_store(self, store_url) -> dict:

        get_store_data = {'store_url': store_url}
        results = self._get_instance_of_mongo_connection().fetch_data(get_store_data, 'style_stree_products')
        response_context = dict()
    ...
    def fetch_product_data_from_style_stree_brand_store_by_name(self, name) -> dict:
    ...
    def insert_store_data_from_style_stree_brand_store(self, store_details):
        url = store_details.get('store_url')
        existing_shop = self.fetch_store_by_url_from_style_stree_brand_store(url)
        ...
        if existing_shop_name:
            self._get_instance_of_mongo_connection().update_data({'store_url': store_details.get('store_url')}, {'$set': store_details}, 'style_stree_stores')

        else:
            self._get_instance_of_mongo_connection().insert_data(store_details, 'style_stree_stores')

    
    def fetch_store_by_url_from_style_stree_brand_store(self,url) -> dict:

        get_store_data = {'store_url': url}
        results = self._get_instance_of_mongo_connection().fetch_data(get_store_data, 'style_stree_stores')
        response_context = dict()
    ...
    def save_brand_creds(self,brand_global_id,store_credentials):
        ...
        default = {
                    ...
                    'private_metadata':{
                        'order_api':'http://test.stylestry.com/api/zammo/orders'
                    }
                }

        brand_cred = BrandCred.objects.update_or_create(brand_id=brand_id,defaults=default)

    def create_brand_from_style_stree_brand(self, shop):
        ...
        brand_info = StyleStreeHelper.create_brand_context_from_style_stree(shop)
    ...
    def create_product_from_style_stree_brand(self, shop):
    ...
    def fetch_tags_product_id_dict(self, brand_name):
        ...
        product_data = self.fetch_product_data_from_style_stree_brand_store_by_name(brand_name)
            ...
            for product in product_list:
                ...
                if tags:
                    ...
                    for tag in tags:
                        ...
                        if tag_name:
                            data[tag_name].append(product.get('product_id'))
    ...
    def resync_price_and_inventory(self):
        ...
        brand_creds = BrandCred.objects.filter(brand__private_metadata__source_name__in=['stylestree','shoetopia']).select_related('brand')
        insts = StyleStreeBrandIntegeration()
        for document in brand_creds:
            try:
                
                existing_shop = self.fetch_store_by_url_from_style_stree_brand_store(document.url)['result']
                for i in existing_shop:
                    shop = i
                    break
                store_name= document.brand.private_metadata.get('source_name') or document.brand.brand_name
                
                response = self.insert_product_data_from_style_stree_brand_store(shop,resync=True)
                ...
                for product in product_data:
                    ...
                    product_id_brands_list.append(StringUtilities.convert_object_to_string(product.get('product_id')))
    ...
    def _create_order_context_from_stylestree(self, order_lines, brand_mappings):
        ...
        data = StyleStreeHelper.prepare_context_for_order(user_email,billing_address,shipping_address,line_items)
    ...
    def _create_order(self,order_context,brand_name):
        ...
        try:
            ...
            order_api = brand_cred.private_metadata.get('order_api')
            api_client = ApiClient(url=order_api)
    ...
    def place_orders_util(self, order_line_obj, brand_name, brand_variant_mappings):
        ...
        order_context = self._create_order_context_from_stylestree(order_line_obj, brand_variant_mappings)
        ...
        if res_data['success']==True:
            ...
            instance.save_order_mapping(order_line_obj, api_response.get('Id'))
            
        return res_data

    def update_order_status(self):
        #using shoetopia since it is a sub brand of stylestree 
        orders = OrderBrandZaamoMapping.objects.filter(brand__private_metadata__source_name__in=['stylestree','shoetopia'],
        ...
        for order in orders:
            order_brand_dict[order.brand.private_metadata.get('source_name')].append(order)
        ...
        for brand_name,order_list in order_brand_dict.items():
            ...
            for order_m in order_list:
                try:
                    ...
                    api = ApiClient(url=f"{order_api}?id={order_m.order_id_brand}")
                    api.get()
    ...
    def update_order_status_from_webhook(self, order_data,brand_id):
        ...
        zaamo_status = StyleStreeHelper.get_brand_to_zammo_order_status(status)
