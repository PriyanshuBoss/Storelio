...
from saleor.external_services.shopify_service.constants import SIZE_KEYS
...
class ShopifyIntegration(BaseIntegration):
    ...
    category = "shopify_uncategorized"
    ...
    def get_attributes(self, options):

        attributes = {}
        for option in options:
            
            if option.get('name').lower() == 'colour':
                option['name'] = 'color'

            if option.get('name').lower() not in ['size', 'color']:
                continue

            attributes[option['name'].lower()] = {"values": option["values"], "is_variant_attribute": True}
    ...
    def _get_color_option(self, options):
        ...
        for option in options:
            ...
            if option.get('name').lower() in SIZE_KEYS or 'size' in option.get('name').lower():
                size_options_list = option.get('values') or []
                size_options.extend(size_options_list)

        return {'color': color_options, 'size': size_options}

    def get_minimal_price_amount(self, variants):
        return min([Decimal(variant.get("price"), ) for variant in variants])
        ...
        for variant in variants:
            ...
            if variant['option1'] == 'Default Title':
                ...
                for value in color.split('$'):
                    if value=='':
                        continue
                    values_excluding_size.append(value)
                
                if values_excluding_size:
                    diff = set(values_excluding_size).difference(set(values))
                    
                    if diff:
                        continue
                
                size_value = set(values).difference(set(values_excluding_size))

                name = None
                
                if size_value:
                    name = '/'.join(list(size_value))
                    _variant_attr['size'] = list(size_value)
            ...
            cost_price = NumberUtilities.convert_string_to_float(variant.get("compare_at_price"))
            price = NumberUtilities.convert_string_to_float(variant.get("price"))
            ...
            fields = {
                "variant_id_brand": variant.get("id"),
                "private_metadata": {},
                "metadata": {},
    ...
    def _prepare_product_description_json(self, json_item):
        ...
        attributes = json_item.get('options')
        attribute_list = []

        for att in attributes:
            
            if att['name']=='Title' or att['name']=='Size':
                continue

            attr_string = f"<b>{att['name']}</b> : {', '.join(att['values'])}"
            attribute_list.append(attr_string)

        weight = json_item.get('variants_list')[0].get('weight') if json_item.get('variants_list') else None

        if weight:
            unit = json_item.get('variants_list')[0].get('weight_unit')
            attribute_list.append(f"<b>weight</b> : {weight} {unit}")
            
        #To be removed or uncommented later   
        '''
        vendor = json_item.get('vendor')

        if vendor:
            attribute_list.append(f"vendor : {vendor}")
        '''
        product_type = json_item.get('product_type')

        if product_type:
            attribute_list.append(f"<b>type</b> : {product_type}")
        ...
        if json_item.get("description"):
            ...
            description_string = description + '</br>' + description_string
    ...
    def _get_mapped_data(self,data):
        ...
        variants,min_price = self.get_variants_data(json_item.get("variants_list"), data['product_name'], data['color'])
    ...
    def _create_color_attribute_image(self, variant_list, product_name, image_id_url, options_dict):
        ...
        for variant in variant_list:
            ...
            if variant.get('image_id') and image_id_url.get(variant.get('image_id')):
                
                if not image_id_url.get(variant.get('image_id')) in color_image_dict[value]:

                    color_image_dict[value].append(image_id_url.get(variant.get('image_id')))
                    variant_image_ids.append(variant.get('image_id'))
                            
        if not color_name_dict:
            color_name_dict['default'] = product_name
            color_image_dict['default'] = [image_id_url[id] for id in image_id_url.keys()]
        ...
        for image_id, url in image_id_url.items():
            ...
            if not image_id in variant_image_ids:
                ...
                for color in color_values:
                    ...
                    if not url in color_image_dict[color]:
                        
                        color_image_dict[color].append(url)

        for color in color_values:
            
            if not color_image_dict.get(color):
                color_image_dict[color]=list(image_id_url.values())
        return color_name_dict,color_image_dict


    def _get_image_id_url_dict(self,images):
        images_dict = dict()

        for image in images:
            images_dict[image['id']] = image['src']

        return images_dict

    def base_product_mapper(self, json_item):
        
        if not json_item.get('status')=='active':
            return []
        ...
        product_attributes = self.get_attributes(json_item["options"])

        options_dict = self._get_color_option(json_item["options"])
        ...
        image_id_url_dict = self._get_image_id_url_dict(json_item.get('images'))
        color_name_dict, color_images_dict = self._create_color_attribute_image(json_item['variants_list'], self.clean_product_name(json_item.get('title')), 
    ...
    def delete_variation_if_removed_from_store(self, data):
        
        existing_variants = BrandVariantZaamoMapping.objects.filter(product_id_brand = data['id']).values_list('variant_id_brand','product_zaamo_id')
        ...
        variant_list_data = data['variants_list']
    ...
    def update_inventory_and_price(self,new_product_data, brand_name):
        ...
        variants = self.clean_variant_data_for_update(new_product_data.get('variants_list'))
    ...
    def update_brand_price_record(self,new_product_data, brand_name,existing_product):
        ...
        p_name = new_product_data.get('title')
        new_variants = self.clean_variant_data_for_update(new_product_data.get('variants_list'))
        exiiting_variants = self.clean_variant_data_for_update(existing_product.get('variants_list'))
    ...
    def update_status_in_fulfillment(self, order_brand_id, variant_id_status_dict,variant_id_tracking_dict):
        ...
        variant_tracking_brand_zaamo_dict = {mapping.variant_zaamo_id : variant_id_tracking_dict.get(NumberUtilities.convert_string_to_number(mapping.variant_id_brand)) for mapping in variant_mapping}
        ...
        for order_map in order_mapping:
            ...
            tracking_url,tracking_number,tracking_company = variant_tracking_brand_zaamo_dict.get(order_line.variant_id) or (None,None,None)

            if not order_map.metadata.get('tracking_url') and (tracking_url or tracking_number):
                order_map.metadata.update({'tracking_url':tracking_url,'tracking_number':tracking_number,'tracking_company':tracking_company})
                order_map.save()
            
            if tracking_url:
                order_line = order_map.order_line_zaamo
                order_line.metadata.update({'tracking_url':tracking_url})
                order_line.save()

            ful_line = order_line.fulfillment_line.first()
            
            if (not ful_line.note) and tracking_url:
                ful_line.note=f'Track your Order : {tracking_url}'
                ful_line.save()
            ...
            for fulfillment in fulfillments:
                try:
                    ...
                    if status in ['CANCELLATION_PROCESSED','CANCELLATION_INITIATED']:
