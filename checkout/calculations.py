...
def checkout_cod_charge(
    *,
    checkout: "Checkout",
    lines: Iterable["CheckoutLine"],
    discounts: Optional[Iterable[DiscountInfo]] = None,
) -> "TaxedMoney":
    """Return the total cod charge of all the checkout lines, taxes included.

    It takes in account all plugins.
    """
    calculated_checkout_cod_charge = get_plugins_manager().calculate_cod_charges(
        checkout, lines, discounts or []
    )
    return quantize_price(calculated_checkout_cod_charge, checkout.currency)
