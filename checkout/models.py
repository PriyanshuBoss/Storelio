...
from ..core.models import ModelWithCreateTimestamp, ModelWithMetadata
...
class Checkout(ModelWithMetadata):
    ...
    app_code = models.CharField(max_length=50,default="", blank=True)
    mobile_device_id = models.CharField(max_length=150,default="",blank=True)
...
class CheckoutStore(ModelWithCreateTimestamp):
    token = models.ForeignKey(Checkout, on_delete=models.CASCADE, related_name ='checkoutstore')
    store = models.ForeignKey(store_models.StoreInfo, on_delete=models.CASCADE, related_name ='checkoutstore')

    @staticmethod
    def create_instance(info):
        instance = CheckoutStore()
