...
class PluginsManager(PaymentInterface):
    ...
    def calculate_checkout_total(
        ...
        default_value = base_calculations.base_checkout_total(
            ...
            cod_charge=self.calculate_cod_charges(checkout, lines, discounts),
            discount=checkout.discount,
            currency=checkout.currency,
        )
        round_of_value = math.ceil(default_value.net.amount)
        if round_of_value < 0:
            round_of_value=0

        default_value.net.amount = Decimal(round_of_value)
    ...
    def calculate_cod_charges(
        self,
        checkout: "Checkout",
        lines: Iterable["CheckoutLine"],
        discounts: Iterable[DiscountInfo],
    ) -> TaxedMoney:
        default_value = base_calculations.base_checkout_cod_charge(checkout, lines)
        return quantize_price(
            self.__run_method_on_plugins(
                "calculate_cod_charges", default_value, checkout, lines, discounts
            ),
            checkout.currency,
        )
    ...
    def create_order_id(self, gateway: str, payment_information: "PaymentData"
    ) -> "GatewayResponse":
        method_name = "create_order_id"
        return self.__run_payment_method(gateway, method_name, payment_information)
