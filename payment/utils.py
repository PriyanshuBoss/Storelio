...
def create_payment(
    ...
    if payment.gateway == 'zaamo.payments.razorpay' and not payment.token.startswith(('order_')):
        ...
        if payment.can_authorize() and not last_transaction:
            ...
            payment_data = create_payment_information(
                payment=payment, payment_token=payment.token, store_source=False, additional_data=extra_data)
            response = plugin_manager.create_order_id(payment.gateway, payment_data)
            # response = gateway.create_order_id(payment_data)
            payment.token = response.transaction_id
            payment.save()
    elif payment.gateway == 'zaamo.payments.cashfree' and not payment.token.startswith(('http')):
        last_transaction: Optional[Transaction] = payment.transactions.filter(
           kind=TransactionKind.CAPTURE).last()

        if payment.can_authorize() and not last_transaction:
            plugin_manager = get_plugins_manager()
            cashfree_order_id = dict({"cashfree_order_id": checkout.token,\
                    "return_url":return_url,
                    "customer_phone": checkout.billing_address.phone})
            extra_data.update(cashfree_order_id)
            payment_data = create_payment_information(
                payment=payment, payment_token=payment.token, store_source=False,additional_data=extra_data)
            response = {"raw_response":""}
            cashfree_response_data = json.loads(payment.extra_data)
            if not total.compare(Decimal("0.0")) or payment_token == 'cashfree1_manual_shopify':
                response = {"raw_response":"Barter - Amount = 0 "}   
                payment.token = checkout.token
            else:
                #check if new cashfree api needs to be called
                is_new_cashfree_api = extra_data.get('is_new_cashfree_api')
                response = plugin_manager.create_order_id(payment.gateway, payment_data)
                cashfree_response_data.update(response.raw_response)
                payment.extra_data = json.dumps(cashfree_response_data,cls = DjangoJSONEncoder)

                logger.info(is_new_cashfree_api)
                
                if is_new_cashfree_api:
                    payment.token = response.raw_response["payment_session_id"]
                else:
                    payment.token=response.raw_response["payment_link"]
