...
def resolve_my_orders(info, created, status, **_kwargs):

    if type(info.context.user) is AnonymousUser:
        return models.Order.objects.none()

    qs = models.Order.objects.confirmed()
    qs = filter_orders(qs, info, created, status)
...
def resolve_order_by_checkout_token(token):
    return (
        models.Order.objects.filter(checkout_token=token).first()
    )