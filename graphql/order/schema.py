...
from .types import MyOrder, Order, OrderEvent
...
class OrderQueries(graphene.ObjectType):
    ...
    my_orders = FilterInputConnectionField(
        MyOrder,
        sort_by=OrderSortingInput(description="Sort orders."),
        filter=OrderFilterInput(description="Filtering options for orders."),
        created=graphene.Argument(
    ...
    order_by_checkout_token = graphene.Field(
        Order,
        description="Look up an order by checkout token.",
        token=graphene.Argument(UUID, description="The order's checkout token.", required=True),
    )
    ...
    def resolve_my_orders(self, info, created=None, status=None, **_kwargs):
        return resolve_my_orders(info, created, status)
...
class OrderMutations(graphene.ObjectType):
    ...
    update_fulfillment = UpdateFulfillment.Field()
    ...
    orderline_meta_update = OrderLineMetaUpdate.Field()
    ...
    order_fulfillment_note = UpdateFulfillmentNote.Field()
    ...
    refund_orderline = RefundOrderLine.Field()

    create_order_freshdesk_ticket = CreateOrderFreshdeskTicket.Field()
