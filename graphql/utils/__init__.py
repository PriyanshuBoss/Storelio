...
def _get_selection_fields(selections) -> dict:
    _fields = {}
    for selection in selections:
        if selection.selection_set:
            _fields[selection.name.value] = _get_selection_fields(selection.selection_set.selections)
        else:
            _fields[selection.name.value] = True
    return _fields

def get_requested_fields(info) -> dict:
    if info.operation.operation == 'query':
        selections = info.operation.selection_set.selections
        return _get_selection_fields(selections)
    return {}
