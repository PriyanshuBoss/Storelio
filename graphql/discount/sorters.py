...
class VoucherStoreDealSortField(graphene.Enum):
    UPDATED_AT = ["updated_at"]

    @property
    def description(self):
        if self.name in VoucherStoreDealSortField.__enum__._member_names_:
            sort_name = self.name.lower().replace("_", " ")
            return f"Sort voucher_store_deal by {sort_name}."
        raise ValueError("Unsupported enum value: %s" % self.value)

class VoucherStoreDealSortingInput(SortInputObjectType):
    class Meta:
        sort_enum = VoucherStoreDealSortField
        type_name = "VoucherStoreDeal"
