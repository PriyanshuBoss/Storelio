...
def _get_edges_for_connection(edge_type, qs, args, sorting_fields):
    ...
    try:
        # logger.info(args)
        # logger.info(qs.model.__name__)
        matching_records = list(qs)
        # logger.info(qs.first())
        # logger.info("\n")
    except Exception as e:
        logger.info(e.args)
        logger.info("From exception block in connections")
        logger.info(args)
        logger.info(qs.model.__name__)
        logger.info("From exception block end")
        matching_records = []
...
class CountableDjangoObjectType(DjangoObjectType):
    ...
    def get_queryset(cls, queryset, info):
        ...
        # adding context to the queryset object to access in sorters and filters
        # NOTE :: if filters and sorters are used in qraphql query fields then add context 
        # to the queryset object in corresponding resolver.
...
class StoreDjangoObjectType:

    @classmethod
    def get_queryset(cls, queryset, info):
        if not (info.context.user.is_staff or info.context.user.is_superuser) and 'HTTP_X_STORE_ID' not in info.context.META:
            raise ValidationError(" Add x-store-id in request headers")
        
        store_id = info.context.META.get('HTTP_X_STORE_ID')
        brand_member = info.context.user.get_brand_member()
        brand_id = None
        if brand_member:
            brand_id =  brand_member.brand
        setattr(info.context, 'brand_id', brand_id)
        setattr(info.context, 'store_id', store_id)
