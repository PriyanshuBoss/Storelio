...
class CreateToken(BaseMutation):
    ...
    class Arguments:
        mobile_no = graphene.String(required=True, description="mobile no. of a user.")
    ...
    @classmethod
    def _retrieve_user_from_credentials(cls, mobile_no) -> Optional[models.User]:
        user = models.User.objects.filter(mobile_no=mobile_no, is_active=True).first()
    ...
    def get_user(cls, _info, data):
        user = cls._retrieve_user_from_credentials(data.get('mobile_no'))
       
        if not user:
            raise ValidationError(
                {
                    "mobile_no": ValidationError(
