...
from saleor.brand.models import Brand, Commission
...
class BrandByIdLoader(DataLoader):
    context_key = "brand_by_id"

    def batch_load(self, keys):
        commissions = Commission.objects.filter(brand=OuterRef("id")).order_by('-created_at')
        Brands = Brand.objects.annotate(first_commission_percentage=Subquery(commissions.values('commission_percentage')[:1]),
                                        zaamo_commission=Subquery(commissions.values('zaamo_commission')[:1])
                                        ).in_bulk(keys)
        return [Brands.get(brand_id) for brand_id in keys]
...
class ProductByIdLoader(DataLoader):
    ...
    def batch_load(self, keys):
        ...
        products = Product.objects.visible_to_all_user(requestor).in_bulk(keys)
