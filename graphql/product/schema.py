...
# from saleor.graphql.brand.types import ProductGroupingMeta
from saleor.graphql.product.types.products import LandingPageCategory,ProductGroupingMeta, ProductGrouping, ProductTag, ProductTagMapping
from ...core.permissions import ProductPermissions
from ..core.enums import ReportingPeriod
from ..core.fields import BaseConnectionField, BaseDjangoConnectionField, FilterInputConnectionField, PrefetchingConnectionField
...
from .filters import (
    ...
    ProductFilterInput,
    ProductGroupingFilterInput,
    ProductTagFilterInput,
    ProductTypeFilterInput,
    ProductVariantFilterInput,
    BrandZaamoMappingFilterInput,
    SourcingRequestFilterInput
)
...
from .resolvers import (
    ...
    resolve_landing_categories,
    resolve_product_by_slug,
    resolve_product_grouping,
    resolve_product_grouping_mapping,
    resolve_product_grouping_meta,
    resolve_product_tag_mappings,
    ...
    resolve_all_collections,
    resolve_similar_products,
    resolve_fetch_product_collections
)
from .sorters import (
    AttributeSortingInput,
    CategorySortingInput,
    CollectionSortingInput,
    ProductGroupingSortingInput,
    ProductOrder,
    ProductTypeSortingInput,
    SourcingRequestInput,
    ProductCollectionSortingInput
)
from .types import (
    Attribute,
    Category,
...
class ProductQueries(graphene.ObjectType):
    ...
    brand_variant_zaamo_mapping = FilterInputConnectionField(
        BrandVariantZaamoMapping,
         filter=BrandZaamoMappingFilterInput(description="Filtering options for Brand Zaamo Mapping."),
        description="List of variant mappings of band variant and  zaamo variant",
    )
    ...
    collections = FilterInputConnectionField(
        ...
        shuffle = graphene.Argument(graphene.Int, description="Shuffling the stores. Pass an Int value between 1-200 for the shuffling"),
    ...
    my_picks = FilterInputConnectionField(
        Product,
        ...
        description="Returns products available in all collections of a store.",
    )
    ...
    landing_categories = PrefetchingConnectionField(LandingPageCategory, description = "landing page")
    
    all_collections = FilterInputConnectionField(
        Collection,
        filter=CollectionFilterInput(description="Filtering options for collections."),
        sort_by=CollectionSortingInput(description="Sort collections."),
        randomized = graphene.Argument(graphene.Boolean, description="set collection randomization"),
        description="List of all collections.",
    )
    sourcing_request = FilterInputConnectionField(
        SourcingRequest,
        filter = SourcingRequestFilterInput(description = "Filtering options for Sourcing Request"),
        sort_by = SourcingRequestInput(description = "Sort Sourcing Request"),
        description = "List of all Sourcing Request"
    )
    product_tags = FilterInputConnectionField(
        ProductTag,
        filter = ProductTagFilterInput(description="Filtering options for brand collections."),
        description = "List of all Sourcing Request"
    )
    similar_products = DjangoConnectionField(
        Product,
        id=graphene.Argument(
            graphene.ID,
            description="id of product to fetch similar product"
        ),
        view_all=graphene.Argument(
            graphene.Boolean,
            description="to view all products"
        ),
        ...
        tag_ids = graphene.List(
            graphene.ID,
            description="ids of tag for filter"
        ),
        description="List of the shop's products.",
    )
    
    similar_cheap_products = DjangoConnectionField(
        Product,
        id=graphene.Argument(
            graphene.ID,
            description="id of product to fetch similar product"
    ...
    product_tag_mapping = DjangoConnectionField(
        ProductTagMapping,
        productid = graphene.Argument(graphene.ID, description="product id to fetch related tags"),
        description = "List of all Sourcing Request" )

    fetch_product_collections = FilterInputConnectionField(
        Collection,
        sort_by = ProductCollectionSortingInput(description="Sorting input for product collections"),
        productid = graphene.Argument(graphene.ID, required=True, description="product id to fetch the collection."),
        description="List of all collections based on productId."
    )

    product_view = graphene.Field(
        Product,
        id=graphene.Argument(graphene.ID, description="ID of the product.",),
        slug=graphene.Argument(graphene.String, description="Slug of the category"),
        description="Look up a product by ID.",
    )
    product_grouping = FilterInputConnectionField(
        ProductGrouping,
        productid = graphene.Argument(graphene.ID, description="ID of the product.",required=False),
        group_name=graphene.Argument(graphene.String, description='name of product'),
        shuffle = graphene.Argument(graphene.Int, description="Shuffling the products. Pass an Int value between 1-60 for the shuffling"),
        filter = ProductGroupingFilterInput(description="Filtering options for Product Grouping."),
        sort_by = ProductGroupingSortingInput(description="Sorting options for product group."),
        description = "Product groups by product ids"
    )
    product_grouping_mapping = FilterInputConnectionField(
        ProductGrouping,
        filter = ProductGroupingFilterInput(description="Filtering options for Product Grouping."),
        sort_by = ProductGroupingSortingInput(description="Sorting options for product group."),
        description = "Product groupings to fetch products from list of ids"
    )

    product_grouping_meta = graphene.Field(
        ProductGroupingMeta,
        group_id = graphene.Argument(graphene.ID, description="ID of the product.",),
        description = "Product grouping metadata comprising of objects of models from rule ids"
    )

    def resolve_similar_products(self, info, **kwargs):
        return resolve_similar_products(info, **kwargs)

    def resolve_similar_cheap_products(self, info, **kwargs):
        return resolve_similar_products(info,less_price=True, **kwargs)

    def resolve_product_tag_mapping(self, info, **kwargs):
        return resolve_product_tag_mappings(info, **kwargs)
    
    def resolve_fetch_product_collections(self, info, **kwargs):
        return resolve_fetch_product_collections(info, **kwargs)
    ...
    def resolve_my_picks(self, info, **kwargs):

        return resolve_products(info, my_picks=True, **kwargs)
    ...
    def resolve_landing_categories(self, info, **kwargs):
        return resolve_landing_categories(info, **kwargs)

    def resolve_all_collections(self, info, **kwargs):

        return resolve_all_collections(info, **kwargs)

    def resolve_product_view(self, info, id=None, slug=None):
        return resolve_product_view(info, slug)

    def resolve_product_grouping(self, info, productid=None,group_name=None,**kwargs):
        return resolve_product_grouping(info, productid,group_name,**kwargs)

    def resolve_product_grouping_mapping(self, info,**kwargs):
        return resolve_product_grouping_mapping(info,**kwargs)

    def resolve_product_grouping_meta(self, info,group_id=None, **kwargs):

        return resolve_product_grouping_meta(info,group_id, **kwargs)

class ProductMutations(graphene.ObjectType):
    toggle_thrift_store = ToggleThriftStore.Field()
    ...
    sourcing_request_create = SourcingRequestCreate.Field()
    sourcing_request_update = SourcingRequestUpdate.Field()
    thrift_product_create = ThriftProductCreate.Field()
    thrift_product_update = ThriftProductUpdate.Field()
    variant_image_assign = VariantImageAssign.Field()
    variant_image_unassign = VariantImageUnassign.Field()
    add_collection_to_store = AddCollectionToStore.Field()
    product_tag_create = ProductTagCreate.Field()
    product_tag_update = ProductTagUpdate.Field()
    product_group_create_or_update = ProductGroupCreateOrUpdate.Field()
    push_product_to_shopify = pushProductToShopify.Field()
