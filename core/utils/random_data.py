...
from saleor.store.store_utilities import create_default_collection_store, setup_store_for_influencer

from ...account.models import Address, Influencer, User
...
from ...order.models import Fulfillment, Order, OrderLine, OrderStore
...
def create_test_default_collection_store(store_instance, user_instance):
    
    collection_instance = Collection(
        pk=random.randint(200, 500),
    ...
    CollectionStore.create_instance({
...
def create_fake_user(save=True):
    ...
    user = User(
        ...
        mobile_no=randomnumber(10)
    )

    if save:
        user.save()
        user.addresses.add(address)
        influencer_instance =Influencer.create_instance({
            'user_instance': user,
            'instagram_username': user.mobile_no
        })
        user_instance = user
        store_instance = setup_store_for_influencer(user_instance, influencer_instance)
        group = create_group("Full Access", get_permissions(), [user_instance])
        create_test_default_collection_store(store_instance, user_instance)
...
def create_orders(how_many=10):
    ...
    for _ in range(how_many):
        ...
        store_filter = StoreInfo.objects.all()
        store_count  = store_filter.count()
        index = random.randint(0, store_count)
        
        if index < store_count:
            OrderStore.create_instance({
                'order_instance': order,
                'store_instance': store_filter[index]
            })
        yield "Order: %s" % (order,)
