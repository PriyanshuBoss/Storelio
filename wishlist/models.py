...
from saleor.core.models import ModelWithActiveState, ModelWithCreateTimestamp, ModelWithCreateUpdateTimestamp

from ..account.models import User
from ..product.models import Product, ProductVariant


class Wishlist(ModelWithCreateUpdateTimestamp, ModelWithActiveState):
    token = models.UUIDField(default=uuid.uuid4, unique=True, editable=False)
    user = models.ForeignKey(
        User, related_name="wishlist", on_delete=models.CASCADE, blank=True, null=True
    )
    store = models.ForeignKey("store.storeInfo", on_delete=models.CASCADE, null=True, blank=True)
...
class WishlistItem(ModelWithCreateTimestamp):
